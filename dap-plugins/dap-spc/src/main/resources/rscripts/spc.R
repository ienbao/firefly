...
A3 <- c(0, 2.658679313, 1.954409883, 1.628102349, 1.42729869, 1.287128109, 1.181915842, 1.099095543, 1.031660633, 0.975350352,
...
A4 <- c(0, 1.88, 1.186756608, 0.796260321, 0.691008453, 0.548090246, 0.509077889, 0.432907313, 0.411784512, 0.361843467, 0.35006864, 0.315788269, 0.30752938, 0.282401077, 0.275972373)

B3 <- c(0, 0, 0, 0, 0, 0.030363634, 0.117688814, 0.185085438, 0.23913584, 0.283701688,
...
B4 <- c(0, 3.266529638, 2.568168201, 2.266044949, 2.088993879, 1.969636366, 1.882311186, 1.814914562, 1.76086416, 1.716298312,
...
d2 <- c(0, 1.128, 1.693, 2.059, 2.326, 2.534, 2.704, 2.847, 2.970, 3.078,
...
intspc.normal.xbarControlLine.fun <- function(x, avg, rcl, scl, i) {
    slength <- length(na.exclude(x))
    return(avg + i * (A2[slength] * rcl + A3[slength] * scl) / 2)
}

intspc.normal.medianControlLine.fun <- function(x, avg, rcl, i) {
    slength <- length(na.exclude(x))
    return(avg + i * A4[slength] * rcl)
}
...
intspc.normal.getPCResult <- function(x, SubGroupSize, usl = NaN, lsl = NaN, RCenterLine = NaN, XBCenterLine = NaN, Mean = NaN) {
    ...
    if (length(x) > 1 && ! is.nan(SubGroupSize)) {
        ...
        RSD <- RCenterLine / d2[SubGroupSize]
        if (! is.na(lsl)) {
...
intspc.normal.getRangeCResult <- function(x, SubGroupSize, RCenterLine = NaN) {
    ...
    if (exists('SubGroups') && ! is.nan(CenterLine)) {
        UCL <- apply(SubGroups, 2, intspc.normal.controlLine.fun, avg = CenterLine, constants = D4)
        LCL <- apply(SubGroups, 2, intspc.normal.controlLine.fun, avg = CenterLine, constants = D3)
    }
...
intspc.normal.getSDCResult <- function(x, SubGroupSize, SDCenterLine = NaN) {
    ...
    if (exists('SubGroups') && ! is.nan(CenterLine)) {
        UCL <- apply(SubGroups, 2, intspc.normal.controlLine.fun, avg = CenterLine, constants = B4)
        LCL <- apply(SubGroups, 2, intspc.normal.controlLine.fun, avg = CenterLine, constants = B3)
    }
    Result <- list(Values = Values, CenterLine = CenterLine, UCL = UCL, LCL = LCL)
    return(Result)
}
